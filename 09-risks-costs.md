...
|4 |Crawler maintenance burden (4 chains)                      |**High**             |Stage 1+  |Isolated crawler classes; monitoring via `oCk_si_crawl_runs` + `/health` alerts; **5–10 h/month time budget** (a *time* cost, not money — §4) (D §3/§12, arch. §9)                                                                                                                                                             |
...
|10|Data loss / no point-in-time recovery on shared hosting    |**Medium**           |Stage 1   |Host backups + weekly `mysqldump` of `oCk_si_*` + periodic off-host download; RPO = “since last dump” — **accepted at this scale**, revisited at the managed-DB move which brings provider PITR (§5, arch. §9, 08 §3.3/§5)                                                                                                     |
...
|17|Shared-host disk exhaustion (`raw_offers` growth)          |**Low**              |Stage 1   |**8-week prune** of `oCk_si_raw_offers` via `bin/prune.php`; price history retained (cheap at this scale) (arch. §6.3, 04 §6.3, `05`)                                                                                                                                                                                          |
...
|20|Stage-3 provider lock-in (managed DB undecided)            |**Low** (deferred)   |Stage 3   |Provider deliberately **open** (PlanetScale / Supabase / RDS — 03 §5/§7, 08 §5); clean `oCk_si_*` tables + PDO repos keep the move portable (04 §2.1, 08 §4)                                                                                                                                                                   |
...
- **What is backed up, and how.** SuperHosting’s own host-managed backups, **plus** a
  **weekly `mysqldump` of `oCk_si_*`** to the home directory via cron (arch. §9),
...
- **Retention (the prune job, risk lens).** `oCk_si_raw_offers` pruned at **8 weeks**
