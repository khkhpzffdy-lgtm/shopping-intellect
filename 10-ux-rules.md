...
|**Catalog**                 |List of category buckets (`GET /categories`, unchanged taxonomy from §4.0b — fixed, no user create/edit/delete of buckets themselves). Tapping a bucket opens **bucket detail**: the owner's own UserProduct/StoreProduct rows attached to that bucket (owner-scoped same as favorites/recent — own + family once `§2.4` ships, never a stranger's), with create/edit/archive/add-to-list actions. **No prices, no offers** (still not the future Offers tab) — amended 2026-06-18, see `decisions.md` "Catalog becomes 'browse my products'" (amends the 2026-06-17 "no connection to list-adding" clause).|
